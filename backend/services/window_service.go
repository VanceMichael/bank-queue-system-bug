...
func CallNextQueue(windowID string) (*models.QueueNumber, error) {
	...
	var allWaitingQueues []*models.QueueNumber
	...
	for _, businessType := range window.BusinessTypes {
		...
		for _, id := range queueIDs {
			...
			if queue.Status == models.QueueStatusWaiting {
				allWaitingQueues = append(allWaitingQueues, queue)
			}
		}
	}

	if len(allWaitingQueues) == 0 {
		return nil, fmt.Errorf("no waiting queues")
	}

	sort.Slice(allWaitingQueues, func(i, j int) bool {
		if allWaitingQueues[i].Priority != allWaitingQueues[j].Priority {
			return allWaitingQueues[i].Priority > allWaitingQueues[j].Priority
		}
		return allWaitingQueues[i].CreatedAt.Before(allWaitingQueues[j].CreatedAt)
	})

	nextQueue := allWaitingQueues[0]
